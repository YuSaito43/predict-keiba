...
for year in years:
    url = f"https://keiba.yahoo.co.jp/schedule/list/{year}/?grade_one=1"
    ...
    flag = 0
    for con in urllist:
        url = "https://keiba.yahoo.co.jp" + con
        text = requests.get(url, headers=headers)
        text.encoding = text.apparent_encoding
        ssoup = bs(text.text, "lxml")
    
        types = ssoup.find('p', id="raceTitMeta").text.split(' | ')[0].split(' ')[0].split('・')[0]    #種別
        rotate = ssoup.find('p', id="raceTitMeta").text.split(' | ')[0].split(' ')[0].split('・')[1]   #回り方
        length = ssoup.find('p', id="raceTitMeta").text.split(' | ')[0].split(' ')[-1]                 #距離
        weather = ssoup.find('p', id="raceTitMeta").find_all('img')[0].get('alt')                      #天気
        condition = ssoup.find('p', id="raceTitMeta").find_all('img')[1].get('alt')                    #状態
        test = ssoup.find('table', id="raceScore").find_all('tr')
        alllist = []
        for i in range(1, len(test)):
            ...
            umaURL = 'https://keiba.yahoo.co.jp' + temp[3].find('a').get('href')
            uma_text = requests.get(umaURL, headers=headers)
            uma_text.encoding = uma_text.apparent_encoding
            uma_soup = bs(uma_text.text, "lxml")
            if len(uma_soup.find_all('table', id='dirUmaBlood')) == 0:
                tlist.append('-')
                tlist.append('-')
            else:
                parents_list = uma_soup.find_all('table', id='dirUmaBlood')[0].find_all('td')
                parents = []
                for pcon in parents_list:
                    if pcon.get('rowspan') == '4':
                        parents.append(pcon.text)
                tlist.append(parents[0])   #父親
                tlist.append(parents[1])   #母親
            if re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[0][0] =='せ':
                tlist.append(re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[0][:2])
                tlist.append(re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[0][2:])
            else:
                tlist.append(re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[0][0])   #性別
                tlist.append(re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[0][1:])  #年齢
            tnum = re.sub(r'[\n\t\s]', '', temp[3].find('span').text).split('/')[1]
